...
class LocationService {
  ...
  /// Returns null if no results found or the result is outside Mauritius.
  Future<Location?> getCoordinatesFromAddress(String address) async {
    try {
      final locations = await locationFromAddress('$address, Mauritius');
      if (locations.isEmpty) return null;
      final loc = locations.first;
      if (!isWithinMauritius(loc.latitude, loc.longitude)) return null;
      return loc;
    } catch (e) {
      AppLogger.error('Geocoding failed for "$address"', e);
      return null;
    }
  }
